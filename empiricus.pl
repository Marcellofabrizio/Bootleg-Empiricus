...
investir(Nome, Mes, Ano) :-
    Nome == cdb,
    viabilidade_cdi(Mes, Ano, 3, Media),
    taxa_cdi(Mes,Ano,Atual),
    Media <= Atual,
    write('comprar cdb, taxa crescente!').
investir(Nome,Mes, Ano) :-
    Nome == cdb,
    write('vender cdb, taxa decrescente!'),
    %apresenta uma alternativa, cuidar para nao criar loop infinito.
    investir(tesouro, Mes, Ano).
investir(Nome, Mes, Ano) :-
    Nome == acoes,
    viabilidade_ibovespa(Mes, Ano, ValorPassado, ValorCorrente),
    ValorPassado =< ValorCorrente,
    write('comprar acoes na Ibovespa, taxa crescente!').
investir(Nome, _, _) :-
    Nome == acoes,
    write('vender acoes na Ibovespa, taxa crescente!'),
	%apresenta uma alternativa, cuidar para nao criar loop infinito.
    investir(dolar, Mes, Ano).
...
% Se a tendência for crescente,  ValorPassado < ValorCorrente.
viabilidade_ibovespa(Mes, Ano, ValorPassado, ValorCorrente) :-
